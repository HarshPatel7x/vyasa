...
*(INDEX.md router renamed to README.md by D22 — README-as-router: retire INDEX.md, shard root README into docs/; the reverse-chron format is unchanged.)*
...
*(Executed WITH MODIFICATIONS by D22 — README-as-router: retire INDEX.md, shard root README into docs/: the shards became README.md routers rather than INDEX.md, and the decisions log went to a single docs/decisions.md rather than per-decision files under docs/decisions/.)*
...
3. Every directory or project under this tree carries a `README.md`. New default-load shard `rules/readme-convention.md` captures the rule. Project-scope only for now; may lift to global later. *(Scoped by D22 — README-as-router: retire INDEX.md, shard root README into docs/: refined to the leaf-vs-routing-vs-mixed README shape.)*
...
### D22. README-as-router: retire INDEX.md, shard root README into docs/
**Decision:** One entry-doc filename everywhere — `README.md`; `INDEX.md` is retired. A README takes one of two shapes by what its directory holds: a **routing** README (holds other docs/shards — a thin *what-this-is* plus a router to the contents; e.g. `rules/`, `docs/`) or a **leaf** README (holds artifacts — the documentation itself, narrative inline; e.g. `hooks/`, `.github/`); a **mixed** directory routes (tie-breaker). The four former `INDEX.md` routers (`rules/`, `notes/`, `workitems/`, `workitems/plans/`) are renamed to `README.md`. The root `README.md` is thinned to identity + a router, and its substantial sections are sharded into `docs/` (`overview.md`, `structure.md`, `decisions.md`, with `docs/README.md` as the routing README). The Decisions log itself moves to `docs/decisions.md`.
**Why:** Three weaknesses in the old two-filename split. (1) The README rule ("every directory carries a `README.md`") contradicted the four `INDEX.md`-only folders — a standing violation noted in `D17 — Workitems become a folder; open/done split; plan-before-build gate`. (2) The "worked-in-directly" criterion for which folders needed a README was fuzzy and mislabelled `rules/` (INDEX-only yet edited constantly). (3) **GitHub, IDEs, and file browsers auto-render `README.md` but not `INDEX.md`**, so the four INDEX folders silently lost the very affordance the README rule exists to provide. The leaf/routing/mixed trichotomy replaces the fuzzy criterion with a rule keyed on what a directory holds. The Decisions log moves to `docs/decisions.md` because it is the only monotonically-growing section (append-only, never deleted) — leaving it in the README would re-create the bloat this change exists to prevent. That move re-points the `Touches: D<N>` enforcement in `hooks/commit-msg` (and the CI `COMMIT_REF` seam) from `README.md` to `docs/decisions.md` — a one-time cost far cheaper than perpetual growth. `rules/island.md`'s "bible" principle is amended: the bible is now the `README.md` + `docs/` tree, with every structural decision in `docs/decisions.md`.
