...
- [2026-06-03-token-usage-plan.md](2026-06-03-token-usage-plan.md) — Planned (not built) the per-session token-usage logging workitem, grounded in empirical `jq` investigation of Claude Code's transcript files and revised after three parallel adversarial reviews (correctness / scope-safety / completeness). Build blocked on a user scope decision (project vs global).
  Lesson: the diverse-lens panel caught two holes I'd have shipped — a parent-only reader blind to subagent cost (those tokens live in a separate `…/<session>/subagents/` folder) and a `jq -s` parse that silently logs nothing when the live transcript's last line is mid-write; neither is catchable by an offline test on saved transcripts. Verified both reviewer claims myself before folding them in, which corrected my own "subagent location unknown" to "confirmed" — I had under-searched and declared a false negative.

- [2026-06-03-commit-msg-set-e-hardening.md](2026-06-03-commit-msg-set-e-hardening.md) — Shipped three one-line fixes to `hooks/commit-msg` (PRs #22/#23/#24, squash-merged), all the same root cause: under `set -euo pipefail`, a command exiting non-zero inside a command-substitution assignment aborts the hook before its friendly guard runs, so a rejected commit prints nothing. Fixes: `|| true` on two grep pipelines + `--root` on the `diff-tree` call.
  Lesson: one fix's adversarial review surfaced the other two siblings; the same bug had a hidden SIGPIPE face (only visible at 60k matching lines); and reachability beat theory — the comment-only abort's real trigger is `git commit -m '#42 …'` (because `-m` doesn't strip `#` lines), not an abstract all-comment template.
