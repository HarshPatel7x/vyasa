...
- [ ] **Switch repo to squash-merge for structured mainline commits.** Flip the repo to squash-merge with the squash commit subject = the (already-validated) PR title, so every commit on `main` is a structured one-liner and the `Merge pull request #N…` wording disappears. → plan: [`plans/squash-merge.md`](plans/squash-merge.md)
