...
- [commit-msg-empty-dcode-abort.md](commit-msg-empty-dcode-abort.md) — plan to fix the `hooks/commit-msg` silent abort (empty error under `set -e`+`pipefail`) when a `Touches:` footer cites a `D<N>` with no matching heading; one-line `|| true` fix so the friendly diagnostic prints. Status: built + shipped (2026-06-03); two adversarial-review checkpoints cleared, nine-case verification green.
- [diff-tree-root-flag.md](diff-tree-root-flag.md) — plan to add `--root` to `readme_changed()`'s `git diff-tree` so a `docs/decisions.md` born in a repo's root commit is seen by the CI-seam staged-file check (was false-failing "not in staged changes"). Status: built + shipped (2026-06-03); two adversarial-review checkpoints cleared, six-case verification green.
- [commit-msg-all-comment-abort.md](commit-msg-all-comment-abort.md) — plan to fix the line-36 silent `set -e` abort when a commit message is all `#`-comments (reachable via `git commit -m '#…'`); one-line `|| true` so the subject-format check fails loudly instead. Status: built + shipped (2026-06-03); two adversarial-review checkpoints cleared, seven-case verification green.
